...
imp.reload(CreateElement) #Load library
...
class Element():
    def __init__(self , num  , nodeA , nodeB  , startingLoadpath , finalLoadpath, elementType, deformation, orderOfDeformation):
        ...
        if self.elementType != 2:
            oE = CreateElement.CreateMemberInAnimation(str(self.num), (x, y, z), self.calcLength(),(a1, a2, a3),self.get_elementType())
            self.member = oE.geometricalObject()
            ...
            oE = CreateElement.CreateGapInAnimation(str(self.num), (x, y, z), self.calcLength(),(a1, a2, a3))
            self.member = oE.geometricalObject()
